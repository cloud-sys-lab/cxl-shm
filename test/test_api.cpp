    ...
    CHECK_BODY("t1 to t2") {
        ...
        std::thread t3(consumer_wrc, queue_offset3, std::ref(offset_3), std::ref(t_receiver3));
         for (int i = DATA_SIZE_BLOCK; i <= DATA_SIZE_MESSAGE; i+=DATA_SIZE_BLOCK) {
            ...
            CXLRef r1 = shm.cxl_malloc_wrc(i, 0);
        ...
        // std::cout << "t_all :" << t_all  << ",t_real_receive" << t_real_receive1 << ",t_send" << t_send  << ",t_receive_2" << t_receive_2 << "，status" << status1 <<"，r1.get_tbr()" << r1.get_tbr() << std::endl;
